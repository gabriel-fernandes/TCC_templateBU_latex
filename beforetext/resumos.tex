...
\newcommand{\imprimirenglishabstract}{%
    ...
    \begin{resumo}[Abstract]
        ...
        In the Digital Substation context, the Merging Units have a key role in the interface between the instrumentation transformers and the Inteligent Electronic Devices (IEDs). Aiming to keep the high fidelity of this interface regarding the current and voltage signals received by the transformers, the packets created and transmitted by the Merging Units should have minimal errors. This works proposes a calibration methodology for the analog acquisition boards of this devices, intending to reduce the magnitude errors and keep the acquisition channels within the maximum ratings provided by the manufacturer. Using different types of calibration methods, it was possible to get sufficient results for the current channels with a good margin. For the voltage channels, the results were not sufficient to keep the errors below the maximum allowed, what shows an expressive non-linearity in this type of interface. For this case, higher order calibrations were proposed as further studies.
